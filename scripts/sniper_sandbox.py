...
def main():
    ...
    # Resolve symbols (CSV → list of full symbols)
    symbols = resolve_symbols(args.symbol)

    # Per-symbol scouts and triggers (independent state)
    scouts: dict[str, SniperScout] = {}
    triggers: dict[str, SniperTrigger] = {}
    prev_metrics: dict[str, dict | None] = {}
    for sym in symbols:
        scouts[sym] = SniperScout(sym)
        triggers[sym] = SniperTrigger()
        prev_metrics[sym] = None

    sym_list = ", ".join(symbols)
    logger.info(f"--- Sniper Sandbox Initialized: {sym_list} (Path: {data_root}, Continuous: {args.continuous}) ---")

    try:
        while True:
            for sym in symbols:
                # 1. Harvest Data (No Images)
                result = scouts[sym].scout()
                metrics = result.metrics

                if not metrics:
                    logger.warning(f"SniperSandbox [{sym}]: Scout returned empty metrics. Skipping.")
                    continue

                # 2. Evaluate Matrix
                is_noteworthy, t_type, reason = triggers[sym].evaluate(metrics, prev_metrics[sym])
                ...
                if is_noteworthy:
                    ...
                    logger.info(f"[{sym}] Trigger Detail: {reason}")
                    triggers[sym].set_triggered(t_type)
                    ...
                    if args.email:
                        ...
                        if config.enabled:
                            ...
                            subject = f"🔫 [SANDBOX] Sniper Trigger: {sym} | {t_type}"
                            ...
                            <body>
                                <div class="container">
                                    <div style="text-align: center; margin-bottom: 25px; border-bottom: 2px solid #f1f5f9; padding-bottom: 15px;">
                                        ...
                                        <h1 style="color: #0f172a; margin: 0; font-size: 24px;">{sym} 信号触发</h1>
                                        <p style="color: #64748b; font-size: 13px;">检测时间: {to_html_display(now_ts)}</p>
                                    </div>
                            ...
                            if dispatcher.dispatch(subject, html_body):
                                logger.info(f"Sandbox [{sym}]: Email alert dispatched.")
                            else:
                                logger.warning("Sandbox: Email dispatch failed. Check credentials.")
                else:
                    print(f"[{now_str}] [{sym}] 💤 SLEEPING | No actionable asymmetry.")

                prev_metrics[sym] = metrics
