...
## ⏳ 动态生存模型 (The Temporal Survival Model)

Singularity 不再采用固定的持有模型，而是通过其 **时间止损 (Time-Stop)** 机制，根据波动机制动态调整资本的风险暴露时长。

### 1. 物理衰减因子 (Friction Multipliers)
系统根据市场“政体”自动匹配时间窗口，确保在低效率环境中快速撤出：
- **🏎️ 高速路政体 (Highway: 2.0x)**：趋势极度顺畅，允许持有时间翻倍以捕获最大趋势红利。
- **🚙 标准政体 (Standard: 1.0x)**：基准持有时间。
- **🧨 高潮政体 (Climax: 0.3x)**：波动暴走，系统强制大幅缩短窗口，执行“快进快出”以规避暴力反抽。
- **🌊 枯水政体 (Dead Water: 0.7x)**：市场流动性枯竭，缩短窗口以防止资金在无意义的震荡中磨损。

### 2. 生存方程 (The Survival Equation)
$$Target\_Holding\_Hours = Base\_Friction \times \frac{ATR_{Micro}}{ATR_{Macro}} \times Multiplier_{Regime}$$


---

## 🛡️ 置信度实战处理手册 (Confidence Action Matrix)
> `[GROUND_TRUTH]: config/global_config.yaml`

基于 **80/50 物理损耗边界**，这是系统输出信号的实战执行指南。
...
| **< 50** | **Halt (熔断)** | **自动屏蔽**：逻辑熵增过大，`NEUTRAL` (不交易)。 | 0% (禁止入场) |

### 🔍 真理双重核验 (The Sniper's Double-Check List)
> `[GROUND_TRUTH]: config/strategy_config.yaml`

在点击确认下单前，请执行以下“逻辑对敲”以确保真理收敛：
1.  **`volatility_expansion_ratio` (VR)**：若 VR > **1.5**，意味着进入“扩张政体”；若 VR > **2.0**，进入“混沌政体”。必须大幅减仓或寻找更深避险位。
2.  **`CVD Impulse` (情绪冲量)**：确认 `scout` 捕获到 CVD > **0.35** 的极端偏斜。这意味着物理层存在真实的买卖盘枯竭/爆发。
