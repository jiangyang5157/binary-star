...
```mermaid
graph TD
    subgraph Input_Layer ["1. 物理输入层 (Entropy)"]
        A1[市场拓扑: HVN/LVN/POC]
        A2[情绪流: CVD/清算簇]
        A3[多模态视觉: Macro/Micro]
    end

    subgraph Alignment_Layer ["2. 逻辑对齐层 (Shared)"]
        B{上下文缓存 & 视觉同步}
    end

    subgraph Reasoning_Triad ["3. 对抗推理环 (Loop)"]
        C[Alpha 规划 - High Temp]
        D[零信任物理验证 - Native Math]
        E[否定性审计 - Low Temp]
    end

    subgraph Convergence_Engine ["4. 收敛判定 (Convergence)"]
        F{阈值 OR Max Rounds}
    end
    subgraph Output_Layer ["5. 确定性输出 (Synthesis)"]
        G[状态降维与纪律合成]
        H[最终执行与物理对齐]
    end
    A1 & A2 & A3 --> B
...
```bash
python run_session.py --start T-29d --end T-15d --samples 20
python run_session.py --start T-15d --end T-1d --samples 20
python run_session.py --start T-4d --end T-2d --samples 10
```
