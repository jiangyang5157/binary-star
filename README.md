...
2.  **Agent B (审查员 Reviewer)**:
    ...
    *   **盈亏复盘**: 对比 Trader 的预测逻辑与市场的实际走势，通过 1h 采样量化最大回撤和实际收益。
    *   **深度诊断 (Post-Mortem)**: 为单次交易生成高颗粒度的“交易尸检”报告，解析亏损的底层技术原因或盈利的偶然性，作为 Coach 的原始输入。

3.  **Agent C (教练 Coach)**:
    *   **模式识别 (Pattern Recognition)**: 横向对比多份 Post-Mortem 报告，识别系统性的逻辑缺陷、偏见或风险偏好不当。
    *   **战略优化 (Master Patch)**: 生成具备即效性的“大师级补丁”（结构化指令列表：ADD/REPLACE/REMOVE），确保对 Trader 提示词的修改精准且不产生幻觉。
