...
- **物理挤压 (The Squeeze)**：**逻辑一致性核验。** 辩方 (Session Agent) 负责提出战略蓝图，而控方 (Critic Agent) 执行致命交叉盘问。收敛不是妥协，而是**强制逼近**。一旦发现结构破绽，系统会被强制放入“物理引擎” (MathTools) 进行计算重建（如挤压出极深的 DLE 限价单以满足盈亏比）。
...
| **懦弱与认知僵化** | `[INACTION_BIAS]` <br> `[TREND_STARVATION]` | **机会损失矫正**。当大趋势爆发、结构依然清晰时，若系统给出无理由的退缩，强行责令其入场！ |
| **局部极端衰竭** | `latest_wick_skew` | **针尖探测**。通过 K 线影线偏移度定位局部的流动性真空与反转征兆。 |
