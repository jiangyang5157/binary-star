...
class EvolverSandbox:
    ...
    def replay_audit_with_patch(
        ...
        logger.info(f"replaying session | id={session_id}")
                    ...
                    if t_path:
                        ...
                        for k in keys:
                            if k not in curr or not isinstance(curr[k], dict):
                                logger.warning(f"skipping patch — path node '{k}' not found in {t_path}")
                                path_valid = False
                                break
                            curr = curr[k]
                    ...
                    if path_valid:
                        if target in curr:
                            ...
                            if not found:
                                ...
                                curr[target] = value
                        logger.info(f"applied config patch | key={t_path + '.' if t_path else ''}{target} | value={value}")

        # 2. Prepare Proposed Instructions (Baseline + Patch) - IN-MEMORY ONLY
        instruction_overrides = {}
        if instruction_patch:
            logger.info("distilling instruction refinements in-memory")
            ...
            for agent_name, rel_path in prompt_paths.items():
                ...
                try:
                    ...
                    if agent_refinements:
                        ...
                        logger.info(f"applied {len(agent_refinements)} refinements | agent={agent_name}")
                except Exception as e:
                    logger.warning(f"could not load baseline | agent={agent_name} | path={abs_path} | error={e}")
        ...
        logger.info(f"anchoring audit to historical T1: {historical_t1.isoformat()}")
    ...
    def run_batch_validation(
        ...
        for idx, report in enumerate(audit_reports):
            ...
            logger.info(f"case {idx+1}/{total} | session={session_id}")
