...
4. **THE NEUTRALITY PARADOX**: If NEUTRAL was chosen and the market chopped, praise "Capital Preservation." If NEUTRAL was chosen but a structurally sound move occurred, severely penalize "Opportunity Cost." **EXCEPTION (JUSTIFIED SURRENDER):** If NEUTRAL was strictly forced by protocol mandates (e.g., Missing Data in T0, or a Fatal `[MACRO_CONFLICT]` Veto from the Critic), you MUST waive the Opportunity Cost penalty. Reward risk discipline over anomalous market outcomes. **DATA-DRIVEN WAIVER**: If T0 telemetry contains any 'Unavailable' flags for critical metrics (ls_ratio, cvd_trend, liquidation_clusters), any NEUTRAL stance is automatically a Justified Surrender.
...
| | **Logic Failure**: `mae_stress_level` > 85% OR `mae_atr_ratio` > 1.5. | -50 (High-risk gamble) |
