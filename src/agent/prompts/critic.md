...
| **Retail Squeeze** | `long_short_ratio_micro` > `{long_short_imbalance_ratio}` OR `abs(funding_rate)` > `{funding_extreme_threshold}`. | **[RETAIL_SQUEEZE]** (Demand Vacuum Flip, or use Momentum Entry if `volatility_ratio` > `{volatility_expansion_ratio}`). | **CONSTRUCTIVE** |
