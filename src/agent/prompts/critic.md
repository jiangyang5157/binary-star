...
| **Inaction Bias**| **(Mandatory for NEUTRAL)**: Confluence exists (e.g. Squeeze < `{regime_squeeze_audit_threshold}` + `trend_intensity` > `{regime_trend_intensity_threshold}`, OR `abs(poc_dist_atr)` > `{regime_poc_gravity_atr_distance}` with CVD absorption) but Strategist is Neutral. | **CONSTRUCTIVE** | **[OPPORTUNITY_DENIAL]** (Fix: Flag missed entry and demand Mean-Reversion DLE). |
...
| **Anchor SL Trap**| SL anchored to POC in TRENDING mode, OR SL is placed inside a liquidity vacuum (LVN). **EXCEPTION**: Waive if CVD aligns with the reversal and POC strength > `{regime_poc_confluence_strength}` (POC only). | **CONSTRUCTIVE** | **[LIQUIDITY_VOID]** (Fix: Move SL distal and clear the vacuum). |
