...
| **Flow Violation** | `FLOW_OPPOSES_BIAS` == TRUE AND `ABSORPTION_RISK` == FALSE. **EXEMPTION (Sweep & Fade)**: PASS if anchored at a pierced `liquidation_cluster` AND (`oi_delta_micro` < 0 OR `latest_wick_skew` confirms extreme rejection). | **[FLOW_VIOLATION]** (Fighting aggressive institutional taker flow. Demand Polarity Pivot to align with CVD, or NEUTRAL. EXEMPTED if ABSORPTION_RISK is TRUE, allowing localized contrarian DLE setups). | **TERMINAL** |
