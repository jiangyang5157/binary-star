...
| **Trend Starvation**| `volatility_ratio` > `{volatility_expansion_ratio}` AND `volatility_ratio` <= `{volatility_extreme_ratio}` AND `trend_intensity` > `{trend_intensity_strong}` AND `last_plan.opinion` != "NEUTRAL". | **[TREND_STARVATION]** (Demand shallow pullback or Momentum Entry. DO NOT force deep DLEs). | **CONSTRUCTIVE** |
