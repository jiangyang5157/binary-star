...
| **Trend Starvation**| `vol_expansion_ratio` > `{volatility_expansion_ratio}` AND `vol_expansion_ratio` <= `{volatility_extreme_ratio}` AND `abs(trend_intensity)` > `{trend_intensity_strong}` AND `last_plan.opinion` != "NEUTRAL". | **[TREND_STARVATION]** (Demand shallow pullback or Momentum Entry in the direction of `trend_intensity` sign. DO NOT force deep DLEs). | **CONSTRUCTIVE** |
...
| **Volatility Chop** | `vol_expansion_ratio` > `{volatility_expansion_ratio}` AND `abs(trend_intensity)` < `{trend_intensity_min_expansion}` AND `last_plan.opinion` != "NEUTRAL". | **[VOLATILITY_CHOP]** (Market is in violent chop. Directional trades are gambling. Demand immediate NEUTRAL). | **TERMINAL** |
