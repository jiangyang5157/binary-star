...
4. **CONSTRUCTIVE vs FATAL VETO**: 
   - If you trigger a Veto for a mitigation tag (`[LIQUIDITY_VOID]`, `[ABSORPTION_TRAP]`, `[RETAIL_SQUEEZE]`), you MUST explicitly suggest a **"Deep Limit Entry (DLE)"** in your `hidden_risk` block. Tell the Strategist exactly where the true liquidity sweep will occur.
...
5. **Final Verdict**: Quantify the overall systemic doubt into a `skepticism_score`.
...
{{
    "is_veto": boolean,
    "skepticism_score": 0-100 (0-40: Strong Pass, 41-69: Weak Pass/Optimization Suggested, 70-100: Fatal Veto),
