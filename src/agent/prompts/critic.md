...
| **Retail Squeeze** | `long_short_ratio_micro` > `{long_short_imbalance_ratio}` OR `abs(funding_rate)` > `{funding_extreme_threshold}`. | **[RETAIL_SQUEEZE]** (Demand Vacuum Flip; use Momentum Entry ONLY IF `volatility_ratio` > `{volatility_expansion_ratio}` AND `volatility_ratio` <= `{volatility_extreme_ratio}`). | **CONSTRUCTIVE** |
...
| **Expansion Anomaly** | `volatility_ratio` > `{volatility_expansion_ratio}` AND `volatility_ratio` <= `{volatility_extreme_ratio}` AND `last_plan.opinion` != NEUTRAL. | **[OVER_EXTENSION]** (Demand pivot or wider SL; DO NOT force deep DLEs). | **CONSTRUCTIVE** |
