...
- `HAS_ANCHOR_VIOLATION`: NOT `IS_SL_SHIELDED` OR (The structural anchor is NOT physically BETWEEN `entry` and `stop_loss`) OR (The `stop_loss` is placed at or in front of a `liquidation_cluster`)
