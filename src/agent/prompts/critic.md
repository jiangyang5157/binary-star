...
| **Inaction Bias**| `last_plan.opinion` == "NEUTRAL" AND ( `squeeze_factor` < `{squeeze_audit_threshold}` AND `volume_breakout_ratio` > `{min_vol_participation_ratio}` OR `abs(poc_dist_atr)` > `{poc_gravity_atr_distance}` ). | **[OPPORTUNITY_DENIAL]** (Demand Mean-Reversion DLE or Vacuum Flip). | **CONSTRUCTIVE** |
| **Opportunity Denial**| `volatility_ratio` > `{volatility_expansion_ratio}` AND `volatility_ratio` <= `{volatility_extreme_ratio}` AND `last_plan.opinion` != "NEUTRAL". | **[OPPORTUNITY_DENIAL]** (Demand Momentum Participation or Market Entry). | **CONSTRUCTIVE** |
...
| **Gravity Exhaustion**| `IS_OVEREXTENDING` == TRUE AND (`volume_breakout_ratio` < `{gravity_volume_override_ratio}` OR `abs(poc_dist_atr)` > `{poc_extreme_extension_atr}`). | **[GRAVITY_EXHAUSTION]** (Demand Mean-Reversion DLE or Neutral). | **CONSTRUCTIVE** |
| **Volatility Chop** | `volatility_ratio` > `{volatility_expansion_ratio}` AND `trend_intensity` < `{trend_intensity_min_expansion}` AND `last_plan.opinion` != "NEUTRAL". | **[VOLATILITY_CHOP]** (Market is in violent chop. Directional trades are gambling. Demand immediate NEUTRAL). | **TERMINAL** |
| **Expansion Anomaly** | `volatility_ratio` > `{volatility_expansion_ratio}` AND `volatility_ratio` <= `{volatility_extreme_ratio}` AND `trend_intensity` >= `{trend_intensity_min_expansion}` AND `last_plan.opinion` != "NEUTRAL". | **[OVER_EXTENSION]** (Demand pivot or wider SL; DO NOT force deep DLEs). | **CONSTRUCTIVE** |
| **Volatility Climax** | `volatility_ratio` > `{volatility_extreme_ratio}` AND `last_plan.opinion` != "NEUTRAL". | **[VOLATILITY_CLIMAX]** (Volatility climax. Demand deep DLE for mean-reversion or NEUTRAL. Momentum entries are PROHIBITED). | **CONSTRUCTIVE** |
