...
| **Divergence** | Price holding/rising while `cvd_trend` is DOWNWARD (Passive Absorption), OR Price falling/holding while `cvd_trend` is UPWARD (Passive Distribution). | **[ABSORPTION_TRAP]** (Mitigate: Demand deeper entry). |
| **Weak Breakout**| Price crosses VAH/VAL but `vol_breakout` < 1.2. | **[ABSORPTION_TRAP]** (Mitigate: Wait for liquidity sweep. If volume is stagnant, suggest local micro-anchor, not macro extreme). |
