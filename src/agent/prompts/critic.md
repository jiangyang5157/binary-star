...
- **Shield Integrity**: If `sl_is_shielded: False`, the `stop_loss` is floating in a structural vacuum. Trigger **[ANCHOR_VIOLATION]**. Additionally, you MUST verify that the shielding anchor is located BETWEEN the `entry` and the `stop_loss`. If the anchor is above both (for longs) or below both (for shorts), it provides zero protection; trigger **[ANCHOR_VIOLATION]**.
