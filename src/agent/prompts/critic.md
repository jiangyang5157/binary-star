...
| :--- | :--- | :--- | :--- |
| **Order Physics** | (`last_plan.opinion` == "BULLISH" AND (`last_plan.tactical_parameters.entry` > `current_price` OR `last_plan.tactical_parameters.stop_loss` >= `last_plan.tactical_parameters.entry`)) OR (`last_plan.opinion` == "BEARISH" AND (`last_plan.tactical_parameters.entry` < `current_price` OR `last_plan.tactical_parameters.stop_loss` <= `last_plan.tactical_parameters.entry`)). | **[ORDER_PHYSICS]** (Directional physics violated. BULLISH: current >= entry > SL. BEARISH: current <= entry < SL). | **TERMINAL** |
...
| **Structural Violation** | `last_plan.tactical_parameters.entry` hits a volume vacuum (`vacuum_score` > `{vacuum_risk_score}`). | **[STRUCTURAL_TRAP]** (Entry must be anchored to an HVN, POC, VAH, VAL, or Liquidation Cluster. Move Entry to a valid structural node). | **TERMINAL** |
...
| **Math Violation** | `compliance_verdict.rr_is_valid` == FALSE OR `compliance_verdict.atr_volatility_is_logical` == FALSE. **EXEMPTION**: In Chaos regimes (VEI > `{volatility_extreme_ratio}`), RR violations up to `{chaos_rr_discount}` (as a percentage) are acceptable to accommodate Volatility Adaptive Shielding. | **[MATH_VIOLATION]** (The proposal has failed physical verification. Recalculate SL/Entry to align with `{compliance_verdict}` requirements). | **CONSTRUCTIVE** |
...
**Flow Violation** | `FLOW_OPPOSES_BIAS` == TRUE AND `ABSORPTION_RISK` == FALSE. **EXEMPTION (Sweep & Fade)**: PASS if anchored at a pierced `liquidation_cluster` AND (`oi_delta_micro` < 0 OR `wick_skew_instant` < `{wick_skew_momentum_bearish}` OR `wick_skew_instant` > `{wick_skew_momentum_bullish}`). You MUST explicitly state the numerical values of these metrics in your reasoning to grant this exemption. | **[FLOW_VIOLATION]** (Fighting aggressive institutional taker flow. Demand Polarity Pivot to align with CVD, or `NEUTRAL`. EXEMPTED if `ABSORPTION_RISK` is TRUE, allowing localized contrarian DLE setups). | **CONSTRUCTIVE** |
| **Expansion Anomaly** | `volatility_expansion_index` > `{volatility_expansion_ratio}` AND `volatility_expansion_index` <= `{volatility_extreme_ratio}` AND `abs(trend_intensity)` >= `{trend_intensity_min_expansion}` AND `last_plan.opinion` != "NEUTRAL". | **[OVER_EXTENSION]** (Demand deeper DLE to survive volatility. Widening SL without improving entry price is PROHIBITED. If `abs(trend_intensity)` < `{trend_intensity_strong}`, anchor DLE at the primary POC or nearest HVN to survive mean-reversion whipsaws; DO NOT force the entry to a secondary/distal HVN if the primary POC provides a valid RR. If `abs(trend_intensity)` >= `{trend_intensity_strong}`, shallow pullback DLE is acceptable, but MUST be structurally shielded. **EXEMPTION**: If `FLOW_DOMINANCE_OVERRIDE` == TRUE OR `squeeze_factor` < `{squeeze_threshold}`, shallow pullback DLEs are fully authorized to capture the breakout, do not veto). | **CONSTRUCTIVE** |
...
3. **Structural & Temporal Integrity (The Shield Audit)**: (**SKIP IF OPINION IS NEUTRAL**). Directly audit the physical safety of the plan using snapshots and `math_fact_check`.
    ...
    - **PHYSICAL POSITION CHECK**: You MUST visually verify that the designated structural anchor (HVN/POC/Boundary) is physically located **BETWEEN** the `entry` and the `stop_loss`. If the anchor is above both (for longs) or below both (for shorts), the shield is an illusion; you MUST trigger **[ANCHOR_VIOLATION]**. Furthermore, verify the SL is NOT placed at or in front of a `liquidation_cluster`. If the SL acts as liquidity for a cluster, trigger **[ANCHOR_VIOLATION]**.
