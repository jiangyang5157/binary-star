...
| **Expansion Anomaly** | `volatility_expansion_ratio` > `{volatility_expansion_ratio}` AND `volatility_expansion_ratio` <= `{volatility_extreme_ratio}` AND `abs(trend_intensity)` >= `{trend_intensity_min_expansion}` AND `last_plan.opinion` != "NEUTRAL". | **[OVER_EXTENSION]** (Demand wider SL to survive volatility. If `abs(trend_intensity)` < `{trend_intensity_strong}`, anchor DLE deep at POC/HVN to survive mean-reversion whipsaws. If `abs(trend_intensity)` >= `{trend_intensity_strong}`, shallow pullback DLE is acceptable to maintain trend participation). | **CONSTRUCTIVE** |
