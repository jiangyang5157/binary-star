...
| **Structural Violation** | `last_plan.tactical_parameters.entry` is placed at an LVN or floating in a volume vacuum. | **[STRUCTURAL_TRAP]** (Entry must be anchored to an HVN, POC, VAH, VAL, or Liquidation Cluster. Move Entry to a valid structural node). | **TERMINAL** |
