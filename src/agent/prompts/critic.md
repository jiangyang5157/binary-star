...
| **Opportunity Denial**| `volatility_ratio` > `{volatility_extreme_ratio}` AND `draft_plan.opinion` != NEUTRAL. | **[OPPORTUNITY_DENIAL]** (Demand Momentum Participation or Market Entry). | **CONSTRUCTIVE** |
| **Retail Squeeze** | `long_short_ratio_micro` > `{long_short_imbalance_ratio}` OR `abs(funding_rate)` > `{funding_extreme_threshold}`. | **[RETAIL_SQUEEZE]** (Demand Vacuum Flip or deeper DLE to hunt retail liquidity). | **CONSTRUCTIVE** |
...
2. **Structural Integrity (Math Truth Overlay)**: (**SKIP IF OPINION IS NEUTRAL**). Cross-reference `draft_plan` with `math_fact_check`.
