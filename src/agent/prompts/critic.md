...
5. **MATH SUPREMACY & COORDINATE AWARENESS**: The `[MATH FACT CHECK]` provides `entry_to_sl_atr` (the total risk distance from Entry to SL). **DO NOT confuse this with the Strategist's 0.2x-0.5x ATR structural buffer (which is measured from the Anchor, not the Entry).** Use the Strategist's scratchpad to verify the Structural Anchor distance, and use the system's `entry_to_sl_atr` and `entry_to_tp_atr` strictly to verify the final RR math. VETO if the final RR violates Protocol 3.
...
{{
    ...
    "math_check": "Explicit validation of the Strategist's RR and Stop Loss placement against the math_fact_check (specifically comparing entry_to_sl_atr and the claimed Anchor-to-SL distance). (If opinion is NEUTRAL, output 'N/A - Neutral Stance')."
}}