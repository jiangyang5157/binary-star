...
4. **STRUCTURAL ANCHORING & ULTIMATE FLOOR**: SL must be placed dynamically (**`{stop_loss_buffer_min}`x - `{stop_loss_buffer_max}`x ATR**) beyond a major structural anchor (`POC`/`VAL`/`VAH`) as defined in the **EXECUTION LAW**. 
   - **Regime Awareness**: In `RANGING` regimes with `volatility_ratio` > 1.3 OR in `TRENDING`/`IMBALANCED` regimes, the `POC` is a rotational magnet, not a shield; you MUST anchor SL beyond `VAH`/`VAL` edges or distal HVNs.
   - **Liquidity Shield**: If `volatility_ratio` > 2.0 AND `long_short_ratio` > 2.0, standard ATR buffers fail against liquidation cascades; you MUST anchor SL behind a distal HVN or output `NEUTRAL`.
