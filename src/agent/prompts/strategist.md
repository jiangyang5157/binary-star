...
4. STRUCTURAL ANCHORING: SL must be placed dynamically (**0.2x - 0.5x ATR**) beyond a major structural anchor (POC/VAL/VAH) as defined in the EXECUTION LAW. In RANGING regimes with vol_ratio > 1.3, the POC is a rotational magnet, not a shield; you MUST anchor SL beyond VAH/VAL edges or distal HVNs. Never place an SL in a vacuum (Low Volume Node). If `liquidation_clusters` is `null`, promote **Volume Topography** (HVNs/LVNs) as your absolute structural map.
...
7. REGIME EXECUTION: In RANGING regimes, target Inner-Value nodes (LVNs) if vol_breakout < 1.0; at structural extremes (VAH/VAL), prioritize mean-reversion to the POC. In high-velocity TRENDING regimes (trend_intensity > 0.35, vol_ratio > 2.0), DO NOT mean-revert to the POC; anchor entries to shallow LVNs or Value Area edges to capture momentum.
