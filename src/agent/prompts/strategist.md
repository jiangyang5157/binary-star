...
4. STRUCTURAL ANCHORING: SL must be placed dynamically (**0.2x - 0.5x ATR**) beyond a major structural anchor (POC/VAL/VAH) as defined in the EXECUTION LAW. In RANGING regimes with vol_ratio > 1.3 OR in TRENDING/IMBALANCED regimes, the POC is a rotational magnet, not a shield; you MUST anchor SL beyond VAH/VAL edges or distal HVNs. Never place an SL in a vacuum (Low Volume Node). If `liquidation_clusters` is `null`, promote **Volume Topography** (HVNs/LVNs) as your absolute structural map.
