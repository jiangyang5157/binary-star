...
- **Exhaustion**: This rule overrides all breakout participation protocols.
  - Prohibit BULLISH pivots if `current_price` > `VAH` AND (`oi_delta_micro` contains "-" OR `cvd_trend` == "DOWNWARD").
  - Prohibit BEARISH pivots if `current_price` < `VAL` AND (`oi_delta_micro` contains "-" OR `cvd_trend` == "UPWARD").
...
| **Exhaustion Gap**| `wick_skewness_lookback` vs direction (e.g., > `{regime_wick_skewness_exhaustion}` on Long; < -`{regime_wick_skewness_exhaustion}` on Short). | Analyzed over `{order_flow_lookback_hours}`h. Trigger `[RETAIL_SQUEEZE]`. Also monitor **Momentum Reversal** vs `{regime_wick_skewness_momentum_bullish}` (Bull) / `{regime_wick_skewness_momentum_bearish}` (Bear). |
