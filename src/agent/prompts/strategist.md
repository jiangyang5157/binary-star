...
5. **THE CRITIC ALIGNMENT PROTOCOL**: You MUST inspect the standardized tag in the Critic's `hidden_risk` and act accordingly, regardless of the `is_veto` boolean:
    ...
    - **The Pivot Mandate (Structural Sniping)**: If the tag is `[LIQUIDITY_VOID]`, `[ABSORPTION_TRAP]`, `[RETAIL_SQUEEZE]`, `[OPPORTUNITY_DENIAL]`, or `[VOLATILITY_EXPANSION]`, do NOT automatically surrender. 
        ...
        - **Mandatory Methodology (Better Price Detection)**: 
            - **Recalculation**: Perform a complete **Inverse Risk Engineering** cycle for the new direction.
            - **Anchor Selection**: Contrast the `current_price` against the breached boundary (VAH/VAL). If `current_price` is significantly "better" for the new direction (e.g., HIGHER for Bearish, LOWER for Bullish), you MUST NOT chase the boundary. Instead, search for a **Retest Anchor** or **Distal HVN** that offers superior RR and structural safety. Use the boundary as an entry anchor ONLY if price is already penetrating it or if no better distal anchors are available within tactical range.
            - **Sovereign Exception**: You may ONLY output `NEUTRAL` if the flipped setup still fails the dynamic RR thresholds.
