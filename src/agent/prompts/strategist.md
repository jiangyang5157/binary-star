...
10. **DEFENSIVE LIMIT ORDER PROTOCOL (DLE)**: 
    - **Step 1 (The DLE Execution Tree)**: Execute these checks IN ORDER:
      ...
      2.  **[GRAVITY_FILTER]**: Check `abs(poc_dist_atr)` > `{regime_poc_gravity_atr_distance}`?
          - If YES: ONLY proceed if `volume_breakout_ratio` > `{regime_gravity_volume_override_ratio}`. Otherwise, force `NEUTRAL` (Exit).
      ...
      4.  **[STRUCTURAL_ALIGNMENT]**: If `cvd_trend` aligns with direction OR in high-vol `RANGING`, position at the **NEAREST** high-strength HVN or LVN entry point point rather than behind the anchor.
